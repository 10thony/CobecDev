...
export function PublicNavigation() {
  ...
  const publicComponents = useQuery(
    api.hrDashboardComponents.getPublicComponents,
  );

  // Fallback items if query fails or returns empty
  const fallbackItems = [
    { _id: "fallback-0" as any, path: "/", label: "Dashboard", icon: "Home" },
    {
  ...
  const publicNavItems = (navItems || []).map((item) => {
    // Backward compatibility: older seeds used "/" for procurement.
    const normalizedPath =
      item.path === "/" && item.icon === "Globe"
        ? "/procurement-links"
        : item.path;
  ...
  const navItemsWithDashboard = publicNavItems.some((item) => item.path === "/")
    ? publicNavItems
    : [dashboardNavItem, ...publicNavItems];

  // Add public components from hrDashboardComponents that aren't already in navItemsWithDashboard
  const existingPaths = new Set(navItemsWithDashboard.map((item) => item.path));
  ...
  const allItems = [...navItemsWithDashboard, ...additionalItems].sort(
    (a, b) => {
  ...
  const itemsToRender =
    allItems && allItems.length > 0 ? allItems : fallbackItems;
