...
const isLocalDocker = convexUrl.includes('127.0.0.1') || convexUrl.includes('localhost') || convexUrl.includes(':3210');
...
if (isLocalDocker && adminKey) {
  ...
  console.log('⚠️  Local Docker detected but CONVEX_SELF_HOSTED_ADMIN_KEY not set');
...
if (!existsSync(convexJsonPath)) {
  if (isCloudUrl && deploymentName) {
    // Create a minimal convex.json to avoid configuration prompts
    console.log(`📝 Creating convex.json for deployment: ${deploymentName}`);
    const convexJson = {
      deployments: {
        dev: deploymentName,
      },
    ...
    console.log('📝 No convex.json found - convex dev will use default deployment');
  }
}
