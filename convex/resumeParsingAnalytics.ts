...
// Truncate text for storage (keep first N characters)
function truncateForStorage(text: string, maxLength: number = 10000): string {
  if (text.length <= maxLength) return text;
  return text.substring(0, maxLength) + `\n\n[Truncated - original length: ${text.length} characters]`;
}
...
export const recordAnalytics = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Truncate input/output for storage (keep first 10k chars)
    const truncatedInput = truncateForStorage(args.inputText, 10000);
    const truncatedOutput = truncateForStorage(args.outputJson, 10000);
    
    return await ctx.db.insert("resumeParsingAnalytics", {
